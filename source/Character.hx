...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		switch (curCharacter)
		{
			case 'gf':
				// GIRLFRIEND CODE
				tex = Paths.getSparrowAtlas('ch/GF_assets');
				frames = tex;
			...
			case 'gf-christmas':
				tex = Paths.getSparrowAtlas('ch/christmas/gfChristmas');
				frames = tex;
				animation.addByPrefix('cheer', 'GF Cheer', 24, false);
				animation.addByPrefix('singLEFT', 'GF left note', 24, false);
				animation.addByPrefix('singRIGHT', 'GF Right Note', 24, false);
			...
			case 'gf-car':
				tex = Paths.getSparrowAtlas('ch/gfCar');
				frames = tex;
			...
			case 'gf-pixel':
				tex = Paths.getSparrowAtlas('ch/weeb/gfPixel');
				frames = tex;
			...
			case 'dad':
				...
				tex = Paths.getSparrowAtlas('ch/DADDY_DEAREST');
				frames = tex;
				...
				like = 'dad';
			case 'spooky':
				tex = Paths.getSparrowAtlas('ch/spooky_kids_assets');
				frames = tex;
				...
				playAnim('danceRight');
			case 'mom':
				tex = Paths.getSparrowAtlas('ch/Mom_Assets');
				frames = tex;
			...
			case 'mom-car':
				tex = Paths.getSparrowAtlas('ch/momCar');
				frames = tex;
				...
				playAnim('idle');
			case 'monster':
				tex = Paths.getSparrowAtlas('ch/Monster_Assets');
				frames = tex;
			...
			case 'monster-christmas':
				tex = Paths.getSparrowAtlas('ch/christmas/monsterChristmas');
				frames = tex;
				animation.addByPrefix('idle', 'monster idle', 24, false);
				animation.addByPrefix('singUP', 'monster up note', 24, false);
				animation.addByPrefix('singDOWN', 'monster down', 24, false);
				...
				playAnim('idle');
			case 'pico':
				tex = Paths.getSparrowAtlas('ch/Pico_FNF_assetss');
				frames = tex;
			...
			case 'bf':
				var tex = Paths.getSparrowAtlas('ch/BOYFRIEND', 'shared');
				frames = tex;
			...
			case 'bf-christmas':
				var tex = Paths.getSparrowAtlas('ch/christmas/bfChristmas');
				frames = tex;
				animation.addByPrefix('idle', 'BF idle dance', 24, false);
				animation.addByPrefix('singUP', 'BF NOTE UP0', 24, false);
				animation.addByPrefix('singLEFT', 'BF NOTE LEFT0', 24, false);
				...
				flipX = true;
			case 'bf-car':
				var tex = Paths.getSparrowAtlas('ch/bfCar');
				frames = tex;
				animation.addByPrefix('idle', 'BF idle dance', 24, false);
				animation.addByPrefix('singUP', 'BF NOTE UP0', 24, false);
				animation.addByPrefix('singLEFT', 'BF NOTE LEFT0', 24, false);
				...
				flipX = true;
			case 'bf-pixel':
				frames = Paths.getSparrowAtlas('ch/weeb/bfPixel');
				...
				flipX = true;
			case 'bf-pixel-dead':
				frames = Paths.getSparrowAtlas('ch/weeb/bfPixelsDEAD');
			...
			case 'senpai':
				frames = Paths.getSparrowAtlas('ch/weeb/senpai');
			...
			case 'spirit':
				frames = Paths.getPackerAtlas('ch/weeb/spirit');
			...
			case 'parents-christmas':
				frames = Paths.getSparrowAtlas('ch/christmas/mom_dad_christmas_assets');
			...
			default:
				...
				try {
					charJson = CoolUtil.parseJson(Assets.getText('mods/custom_chars/custom_chars.jsonc'));
				} catch (exception) {
				...
				if (!isError) {
					...
					var animJson = File.getContent("mods/custom_chars/"+Reflect.field(charJson,curCharacter).like+".json");
					...
					var rawPic = BitmapData.fromFile('mods/custom_chars/'+curCharacter+"/"+playerSuffix+".png");
					var tex:FlxAtlasFrames;
					var rawXml:String;
					// GOD IS DEAD WHY DOES THIS NOT WORK
					if (FileSystem.exists('mods/custom_chars/'+curCharacter+"/"+playerSuffix+".txt")){
						rawXml = File.getContent('mods/custom_chars/'+curCharacter+"/"+playerSuffix+".txt");
						tex = FlxAtlasFrames.fromSpriteSheetPacker(rawPic,rawXml);
					} else {
						rawXml = File.getContent('mods/custom_chars/'+curCharacter+"/"+playerSuffix+".xml");
					...
					var tex = FlxAtlasFrames.fromSparrow('mods/BOYFRIEND.png', 'mods/BOYFRIEND.xml');
					frames = tex;
					animation.addByPrefix('idle', 'BF idle dance', 24, false);
					animation.addByPrefix('singUP', 'BF NOTE UP0', 24, false);
					animation.addByPrefix('singLEFT', 'BF NOTE LEFT0', 24, false);
	...
	override function update(elapsed:Float)
	{
		...
		//curCharacter = curCharacter.trim();
		//var charJson:Dynamic = Json.parse(Assets.getText('mods/custom_chars/custom_chars.json'));
		//var animJson = File.getContent("mods/custom_chars/"+Reflect.field(charJson,curCharacter).like+".json");
	...
	public static function getAnimJson(char:String) {
		var charJson = CoolUtil.parseJson(Assets.getText('mods/custom_chars/custom_chars.jsonc'));
		var animJson = CoolUtil.parseJson(File.getContent('mods/custom_chars/'+Reflect.field(charJson,char).like + '.json'));
		return animJson;
	}
}